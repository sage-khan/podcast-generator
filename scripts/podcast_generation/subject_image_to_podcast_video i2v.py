#!/usr/bin/env python3
"""
Generate a silent talking-head video from a single subject image using the Kling 1.6 Pro endpoint.

Usage:
  python scripts/subject_image_to_podcast_video\ i2v.py \
      --image-url https://example.com/subject.jpg \
      --video-prompt "subject talking on a podcast looking at camera" \
      --api-base https://example.com \
      --username admin --password admin1234 \
      --local-storage-path ./media/videos/

Input Parameters:
  --image-url            : URL to the subject’s face image (required)
  --video-prompt         : Prompt text for Kling video generation (default shown below)
  --api-base             : API endpoint for the API server (default: https://example.com)
  --username / --password: API credentials (fallback to env DJANGO_SUPERUSER_*)
  --local-storage-path   : If provided, download the resulting video to this folder
  --reference-image-url  : Optional, repeatable reference images to guide Kling style (max 4)
  --include-lipsync      : After silent-video generation, optionally create a lip-synced video
  --audio-file           : Audio file URL to use for lip-sync (required if --include-lipsync)
  --debug                : Enable verbose logging

Output:
  1. Public-accessible subject image URL (after any uploads/pre-signed changes)
  2. Final silent video URL (and optional local copy if --local-storage-path is set)

Full JSON responses from each API call are displayed during execution and saved to
podcast_video_output.json.

Workflow:
  1. Make the provided image publicly accessible (upload / presign / tmpfiles.io).
  2. Call the Kling 1.6 Pro generate-video endpoint to produce a silent talking-head video.
  3. (Optional) Call the Kling lip-sync endpoint if requested.
"""
import argparse
import os
import sys
...
def poll_kling_job(base, hdrs, job_id, max_polls=60, sleep=10):
    ...
    for i in range(max_polls):
        ...
        # Treat job as complete if status is a success state *or* an output URL is already present.
        if status in ("succeeded", "completed", "done") \
            or ("output_url" in job and job["output_url"]) \
            or ("video_url" in job and job["video_url"]):
...
def poll_kling_lipsync_job(base, hdrs, job_id, max_polls=60, sleep=10):
    ...
    for i in range(max_polls):
        ...
        if status in ("succeeded", "completed", "done") \
            or ("output_url" in job and job["output_url"]) \
            or ("video_output_url" in job and job["video_output_url"]):
            print(f"Job completed successfully!")
            # The API response has direct output_url field
            if "output_url" in job and job["output_url"]:
                return job["output_url"]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Prompt for video generation (Kling)"
    )
    parser.add_argument(
        "--api-base", 
...
def main():
    ...
    # Step 2: Generate a talking-head video (silent) using Kling 1.6 Pro
    try:
        print(f"\n=== STEP 2: Generate a talking-head video (silent) ===")
    ...
    # Optional Step 3: Test lip-sync if requested
